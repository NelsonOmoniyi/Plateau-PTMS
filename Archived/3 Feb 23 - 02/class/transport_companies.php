    ...
    public function verTIN($data){
        ...
        if(!$validation['error'])
        {
            ...
            if ($resArr['status'] == 'success') {
                ...
                return json_encode(array('response_code'=>'200', 'response_message'=>'Valid Tax Identification Number. Please wait loading transport company setup','title'=>$title,'firstname'=>$firstName,'middleName'=>$midName,'surname'=>$Surname,'tin'=>$tin,'mobile'=>$mobile,'address'=>$address, 'id'=>$item, 'name'=>$itemName, 'price'=>$itemPrice));
    ...
    public function saveTCP($data)
    {
        if($data['operation'] == "new")
        {
            ...
            if(!$validation['error'])
            {
                ...
                // // exit;
                // if(!$paid > 0){
                    $insert2 = "INSERT INTO tb_payment_confirmation (payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code) VALUES ('$portal_id', 'Transport Companies Registration', '$amount', '$pending', '$officer', '$name', '$trans_code', '$tin', '$item_code')";
                    $check = $this->db_query($insert2,false);
                    $count = count($check);
                    if ($count > 0) {
                        $insert = "INSERT INTO transport_companies (portal_id, business_name, address, owner_name, cac_reg_no, cap, email_add, phone, tin, status, created, passport, expiry_date, item_code)
                ...
                //         $sql   = "SELECT * FROM transport_companies WHERE tin = '$tin' LIMIT 1";
                //         $result = $this->db_query($sql);
                //         $portal_id = $result[0]['portal_id'];
                //         return json_encode(array('response_code'=>0,'response_message'=>'Registration Successfully', 'port_id'=>$portal_id));
                //     }else{
                //         return json_encode(array('response_code'=>407,'response_message'=>'This TIN '.$tin.' Have Been Used And Payment For This Transport Company Licence Have Not Been Made! <a href="https://techhost7x.accessng.com/plateau_transport/transport_comp1.php">Kindly Click On This Link To Make Payment</a>'));
                //     }
                // }
            }else{
                return json_encode(array("response_code"=>20,"response_message"=>$validation['messages'][0]));
