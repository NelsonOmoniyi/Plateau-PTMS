...
class Dealers extends dbobject
{
    ...
    public function dls_list($data)
    {
        ...
        $columner = array(
            ...
            array( 'db' => 'expiry_date', 'dt' => 12, 'formatter'=> function($d, $row){
    ...
    public function verify_TIN($data){
        ...
        if(!$validation['error'])
        {
            ...
            if ($resArr['status'] == 'success') {
                ...
                return json_encode(array('response_code'=>'200', 'response_message'=>'Valid Tax Identification Number. Please wait loading dealership setup','title'=>$title,'firstname'=>$firstName,'middleName'=>$midName,'surname'=>$Surname,'tin'=>$tin,'mobile'=>$mobile,'address'=>$address, 'id'=>$item, 'name'=>$itemName, 'price'=>$itemPrice));
    ...
    public function saveDLS($data)
    {
        if($data['operation'] == "new")
        {
            ...
            if(!$validation['error'])
            {
                ...
                // // exit;
                // if(!$paid > 0){
                    $insert2 = "INSERT INTO tb_payment_confirmation (payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code) VALUES ('$portal_id', 'Dealership Registration', '$amount', '$pending', '$officer','$business_name', '$trans_code', '$tin', '$item_code')";
                    $check = $this->db_query($insert2,false);
                    $count = count($check);
                    if ($count > 0) {
                        
                        $insert ="INSERT INTO dealership (cat,amount,item_code,portal_id,tin,business_name,address,owner_name,cac_reg_no,sponsor,phone,license_category,list_of_apprentice,license_union,mem_no,agreement,status,passport,created,expiry_date,email) VALUES ('$cat','$amount','$item_code','$portal_id','$tin','$business_name','$address','$owner_name','$cac_reg_no','$sponsor','$phone','$license_category','$list_of_apprentice','$license_union','$mem_no','$agreement','$pending','$full_path','$now','$expDate','$email')";
                ...
                //         $sql   = "SELECT * FROM dealership WHERE tin = '$tin' LIMIT 1";
                //         $result = $this->db_query($sql);
                //         $portal_id = $result[0]['portal_id'];
                //         return json_encode(array('response_code'=>0,'response_message'=>'Registration Successfully', 'port_id'=>$portal_id));
                //     }else{
                //         return json_encode(array('response_code'=>407,'response_message'=>'This TIN '.$tin.' Have Been Used And Payment For This Dealership Licence Have Not Been Made! <a href="https://techhost7x.accessng.com/plateau_transport/dealership1.php">Kindly Click On This Link To Make Payment</a>'));
                //     }
                // }
            }else{
                return json_encode(array("response_code"=>20,"response_message"=>$validation['messages'][0]));
