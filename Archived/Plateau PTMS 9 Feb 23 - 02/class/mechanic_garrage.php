...
class MechanicGarrage extends dbobject
{
    
    public function mech_list($data)
    {
        $table_name    = "mech_garrage";
            ...
            $columner = array(
                ...
                array( 'db' => 'business_name', 'dt' => 2, 'formatter'=>function($d, $row){
                    if ($row['status'] > 0) {
                        if ($row['approved'] > 0) {
                            // return  "$d | <button class='btn btn-success btn-sm' onclick=\"PrintC('".$row['portal_id']."')\">Print Certificate</button>";
                            return "$d | <a href='certificate/spd_certificate.php?id=".$row['portal_id']."&table=mech_garrage' class='btn btn-primary btn btn-sm' target='_blank'><i class='fa fa-print'></i> Print Certificate</a>";
                        } else {
                            return  ''.$d.' | <a class="btn btn-primary btn-sm" onclick="getModal(\'setup/preview_spare_part.php?id='.$row['portal_id'].'&table=mech_garrage\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Approve</a>';
                        } 
                    } else {
                        return  $d;
                    }
    ...
    public function verTIN($data){
        ...
        if(!$validation['error'])
        {
            ...
            if ($resArr['status'] == 'success') {
                ...
                return json_encode(array('response_code'=>'200', 'response_message'=>'Valid Tax Identification Number. Please wait loading mechanig garrage setup','title'=>$title,'firstname'=>$firstName,'middleName'=>$midName,'surname'=>$Surname,'tin'=>$tin,'mobile'=>$mobile,'address'=>$address, 'id'=>$item, 'name'=>$itemName, 'price'=>$itemPrice, 'cac'=>$cac));
    ...
    public function saveMech($data)
    {
        if($data['operation'] == "new")
        {
            ...
            if(!$validation['error'])
            {
                ...
                // if(!$paid > 0){
                    $insert2 = "INSERT INTO tb_payment_confirmation (payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code) VALUES ('$portal_id', 'Mechanic Garage Registration', '$amount', '$pending', '$officer','$name', '$trans_code', '$tin', '$item_code')";
                    $check = $this->db_query($insert2,false);
                    $count = count($check);
                    if ($count > 0) {
                        $insert = "INSERT INTO mech_garrage (portal_id, business_name, address, owner_name, cac_reg_no, cap, email_add, phone, tin, status, created, upload, expiry_date, item_code)
                ...
                // }else{
                    ...
                    //     $sql   = "SELECT * FROM mech_garrage WHERE portal_id = '$portal_id' LIMIT 1";
                    //     $result = $this->db_query($sql);
                    //     $portal_id = $result[0]['portal_id'];
                    //     return json_encode(array('response_code'=>0,'response_message'=>'Registration Successfully', 'port_id'=>$portal_id));
                    // }else{
                    //     return json_encode(array('response_code'=>407,'response_message'=>'This TIN '.$tin.' Have Been Used And Payment For This Mechanic Garage Licence Have Not Been Made! <a href="https://techhost7x.accessng.com/plateau_transport/mech_garrage1.php">Kindly Click On This Link To Make Payment</a>'));
                    // }
                // }
            }else{
                return json_encode(array("response_code"=>20,"response_message"=>$validation['messages'][0]));
    ...
    public function validatePID($data){
        ...
        if ($result == NULL) {
            ...
            // call Validate TIN
            $sql = "SELECT * FROM mech_garrage WHERE portal_id = '$PID'";
            $result = $this->db_query($sql);
            $count = count($result);
            $res = $result[0];
            // var_dump($res);
