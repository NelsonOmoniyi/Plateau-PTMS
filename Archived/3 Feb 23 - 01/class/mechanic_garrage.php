    ...
    public function verTIN($data){
        ...
        if(!$validation['error'])
        {
            ...
            if ($resArr['status'] == 'success') {
                ...
                return json_encode(array('response_code'=>'200', 'response_message'=>'Valid Tax Identification Number. Please wait loading mechanig garrage setup','title'=>$title,'firstname'=>$firstName,'middleName'=>$midName,'surname'=>$Surname,'tin'=>$tin,'mobile'=>$mobile,'address'=>$address, 'id'=>$item, 'name'=>$itemName, 'price'=>$itemPrice));
