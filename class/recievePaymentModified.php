...
class PaymentModified extends dbobject
{
    ...
    public function InitPay($data)
    {
        ...
        $call_back = "https://plateaustatemot.org.ng/slip/special_trade_receipt.php?pid=$pid&&item_code=$item_code&&tbl=$table";
        $sendData = json_encode(array('mda_id' => $MDA_ID, 'mda_reference' => $MDA_re_id, 'tax_item' => $item, 'tin' => $tin, 'amount' => $amount, 'callback_url' => $call_back, 'tax_code' =>$item_code));
        $curl = curl_init();

        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://techhost7x.accessng.com/plateau_transport/initPay.php',
        ...
        $endpoint1_data = json_decode($json_data['endpoint1'], true);
        $status = $endpoint1_data['status'];
        $billing_reference = $endpoint1_data['billing_reference'];
        $mda_reference = $endpoint1_data['mda_reference'];
        $amount = $endpoint1_data['amount'];
        $message_ = $endpoint1_data['message'];

        $endpoint2_data = json_decode($json_data['endpoint2'], true);
        $redirect_to_url = $endpoint2_data['redirect_to_url'];
        $callback_url = $endpoint2_data['callback_url'];
        $message = $endpoint2_data['message'];
        ...
        if ($ren == 'renew') {
            ...
            $sql = "INSERT INTO transaction_table (portal_id,tin, trans_type, transaction_desc, transaction_amount, payment_mode, posted_ip, created, posted_user, payment_gateway, is_processed,trans_query_id) values ('$portal_id','$tin','$item','Registration', '$amount', 'CARD', '$ip','$date', '$officer', 'MONIFY', '1','$billing_reference')";
            $info = $this->db_query($sql, false);

            file_put_contents('validate-tin.txt', '@' . $sql . date("Y-m-d H:i:s") . PHP_EOL, FILE_APPEND | LOCK_EX);

            return json_encode(array("response_code" => "200", "redirect_url" => $redirect_to_url, "callback_url" => $callback_url, "message" => $message));
