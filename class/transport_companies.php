        ...
        $columner = array(
            ...
            array( 'db' => 'status', 'dt' => 10, 'formatter'=> function($d, $row){
                if ($_SESSION['role_id_sess'] != '001') {
                    ...
                    if (!$d > 0) {
                    return  "Pending Payment | <button onclick=\"PrintPage('".$row[portal_id]."')\" class='btn btn-success btn-sm' href='javascript:void(0)'>Print Reciept</button>";
                    } else {
                        return  "PAID ";
                    }
                }
            ...
            if(!$validation['error'])
            {
                ...
                if(!$paid > 0){
                    ...
                    if ( $Tstatus == $processed ) { 
                        ...
                        return json_encode(array('response_code'=>407,'response_message'=>'This TIN '.$tin.' Have Been Used And Payment For This Transport Company Licence Have Not Been Made! <a href="../driving_school_payment.php">Kindly Click On This Link To Make Payment</a>'));
                    }
                }
            }else
            {
