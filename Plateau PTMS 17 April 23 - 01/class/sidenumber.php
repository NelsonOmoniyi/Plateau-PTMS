...
class Sidenumber extends dbobject
{
    ...
    public function verTIN($data)
    {
        ...
        if (!$validation['error']) {
            // call Validate TIN
            $select = "SELECT * FROM vehicle_sidenumbers WHERE plate_number='$plate'";
            // file_put_contents('mine.txt',$select);
            $check = $this->db_query($select);
            $count = count($check);
            if ($count > 0) {
                return json_encode(array('response_code' => '108', 'response_message' => "A sidenumber has been generated for this plate number"));
            } else {
                ...
                if ($resArr['status'] == 'success') {
                    ...
                    return json_encode(array('response_code' => '200', 'response_message' => 'Valid Tax Identification Number.Please wait loading sidenumber setup', 'title' => $title, 'firstname' => $firstName, 'middleName' => $midName, 'surname' => $Surname, 'tin' => $tin, 'mobile' => $mobile, 'address' => $address, 'make' => $make, 'chasis' => $chasis, 'taxPayer' => $taxPayer, 'color' => $color, 'model' => $model, 'plate' => $plate));
                    // }else{
    ...
    public function AuthData($data)
    {
        ...
        // $call_back = "https://plateaustatemot.org.ng/print/printSN.php?tin=$tin";
        $pending = 0;



        // $call_back = "https://techhost7x.accessng.com/callBackSN.php";
        ...
        if ($tin == 'null' || $firstName == 'null' || $middlename == 'null' || $surname == 'null' || $mobile == 'null' || $address == 'null' || $title == 'null' || $plate == 'null' || $chasis == 'null' || $model == 'null' || $make == 'null' || $color == 'null') {
            return json_encode(array('response_code' => 501, 'response_message' => 'Please remove null value from input fields and replace with the appropriate details'));
        } else {
            $validation = $this->validate(
                $data,
                array(
                ...
                array('tin' => 'Tax Identification Number', 'firstname' => 'First Name', 'middlename' => 'Middle Name', 'mobile' => 'Mobile Number', 'surname' => 'Surname', 'address' => 'Address', 'title' => 'Title', 'vehicle_typeid' => 'Trade Union Reg', 'licence_operators' => 'Licence Operator', 'plate' => 'Plate Number', 'chasis' => 'Chasis Number', 'model' => 'Vehicle Model', 'make' => 'Vehicle Make', 'color' => 'Vehicle Color')
            );
            if (!$validation['error']) {
                if ($data['operation'] == "new") {
                    ...
                    if ($result > 0) {
                        if ($result[0]['status'] == '1') {
                            ...
                            // initialize payment
                            $sql1 = "SELECT * FROM plate WHERE plate = '$plate' LIMIT 1";
                            $result1 = $this->db_query($sql1);
                            if (!$result1 > 0) {
                                $insert = "INSERT into plate (Name,plate,chasis,veh_make,veh_color,veh_model,phone) VALUES ('$fullname','$plate','$chasis','$make','$color','$model','$mobile')";
                                $this->db_query($insert, false);
                            }
                            $call_b = "https://techhost7x.accessng.com/callBackSN.php";
                            $res = $payment->intializeTransOP($MDA_ID, $MDA_re_id, $item, $tin, $price, $call_b, $item_code);
                            ...
                            if ($Array['status'] == "failed") {
                                ...
                                $call_back = "https://techhost7x.accessng.com/plateau_transport/print/printSN.php?brn=$billing_ref";
                            }
                            ...
                            $sql = "INSERT INTO transaction_table (portal_id,tin, trans_type, transaction_desc, transaction_amount, payment_mode, posted_ip, created, posted_user, payment_gateway, is_processed,trans_query_id) values ('$SDN','$tin','$item','Registration', '$amount', 'CARD', '$ip','$date', '$officer', 'MONIFY', '0','$billing_ref')";
                            $info = $this->db_query($sql, false);

                            $insert2 = "INSERT INTO tb_payment_confirmation (billing_ref,mda_ref,payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code) VALUES ('$billing_ref','$mda_ref','$SDN', 'Side Number Registration', '$amount', '1', '$officer','$fullname', 'SNR', '$tin', '$item_code')";
                            $this->db_query($insert2, false);
                        ...
                        // initailize payment 
                        $call_bak = "https://techhost7x.accessng.com/callBackSN.php";
                        $res = $payment->intializeTransOP($MDA_ID, $MDA_re_id, $item, $tin, $price, $call_bak, $item_code);
                        $Array = json_decode($res, true);
                        ...
                        if ($Array['status'] == "failed") {
                            ...
                            $amount = $Array['amount'];
                            $message = $Array['message'];
                            $call_back = "https://plateaustatemot.org.ng/print/printSN.php?brn=$billing_ref";
                        }
            ...
            array(
                ...
                'formatter' => function ($d, $row) {
                    if ($_SESSION['role_id_sess'] != '001') {
                        if (!$d > 0) {
                            return '<a class="btn btn-primary btn-sm" onclick="getModal(\'setup/retry_payment.php?id=' . $row['side_number'] . '\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Make Payment</a> Pending Payment';
                        } else {
    ...
    public function renew($data)
    {
        ...
        if (!$validation['error']) {
            ...
            $sql2 = "UPDATE vehicle_sidenumbers SET status = '0' WHERE tax = '$tin'";
            ...
            if ($count > 0) {
                ...
                $MDA_ID = "128";
                $MDA_re_id = substr(str_shuffle(base64_encode(openssl_random_pseudo_bytes(32))), 0, 10);
                $item = "Vehicle side number renewal";
                $item_code = $vehicle_typeid;
                $call_back = "https://techhost7x.accessng.com/callBackSN.php";
                ...
                $renewSql = "UPDATE vehicle_sidenumbers SET brn = '$billing_ref' WHERE side_number = '$sidenumber' AND amount='$amount'";
    ...
    public function retry($data)
    {
        ...
        if (!$validation['error']) {
            ...
            if ($count > 0) {
                ...
                $renewSql = "UPDATE vehicle_sidenumbers SET expiry_date = '$expiryDate' WHERE side_number = '$sidenumber' AND amount='$amount'";
