    ...
    public function verTIN($data)
    {
        ...
        if (!$validation['error']) {
            ...
            if ($resArr['status'] == 'success') {
                ...
                return json_encode(array('response_code' => '200', 'response_message' => 'Valid Tax Identification Number. Please wait loading spare part dealership setup', 'title' => $title, 'firstname' => $firstName, 'middleName' => $midName, 'surname' => $Surname, 'tin' => $tin, 'mobile' => $mobile, 'address' => $address, 'id' => $item, 'name' => $itemName, 'price' => $itemPrice, 'cac' => $cac));
    ...
    public function saveSpare($data)
    {
        ...
        if ($data['operation'] == "new") {
            ...
            if (!$validation['error']) {
                ...
                $item = "Spare Parts Dealership Registration";
                ...
                if (!$Array['status'] == "success") {
                    ...
                    $insert2 = "INSERT INTO tb_payment_confirmation (billing_ref,mda_ref,payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code, t_table,expiry_date) VALUES ('$billing_ref','$mda_ref','$portal_id', 'Spare Parts Dealership Registration'Spare Parts Dealership Registration, '$amount', '$pending', '$officer','$name','$trans_code', '$tin', '$item_code','spare_parts','not set')";
                    $check = $this->db_query($insert2, false);
                    $count = count($check);

                    if ($count > 0) {
                        $insert = "INSERT INTO spare_parts (portal_id, business_name, address, owner_name, cac_reg_no, cap, email_add, phone, tin, status, created, passport, expiry_date, item_code,amount,billing_ref)
