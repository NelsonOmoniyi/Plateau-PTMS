    ...
    public function saveMech($data)
    {
        if ($data['operation'] == "new") {
            ...
            if (!$validation['error']) {
                ...
                $item = "Mechanic Garage Registration";
                ...
                if (!$Array['status'] == "success") {
                    ...
                    $insert2 = "INSERT INTO tb_payment_confirmation (billing_ref,mda_ref,payment_code, trans_desc, trans_amount, trans_status, officer,client_fullname,trans_desc_code, tin, item_code,t_table,expiry_date) VALUES ('$billing_ref','$mda_ref','$portal_id', 'Mechanic Garage Registration', '$amount', '$pending', '$officer','$name', '$trans_code', '$tin', '$item_code','mech_garrage','not set')";
                    $check = $this->db_query($insert2, false);
                    $count = count($check);
                    if ($count > 0) {
                        $insert = "INSERT INTO mech_garrage (portal_id, business_name, address, owner_name, cac_reg_no, cap, email_add, phone, tin, status, created, upload, 'not set', item_code,amount,billing_ref)
                        VALUES ('$portal_id', '$name', '$address', '$owner_name', '$CAC', '$capacity','$email','$phone', '$tin', '$pending', '$now', '$full_path', '$expDate', '$item_code', '$amount','$billing_ref')";
