...
class Payment extends dbobject
{
    ...
    public function InitPay($data)
    {
        ...
        if ($ren == 'renew') {
            ...
            $tin = $result[0]['tin'];
            $billing_ref = $result[0]['billing_ref'];
            $call_back = "https://techhost7x.accessng.com/slip/special_trade_receipt.php?brn=$billing_ref";
        }
        ...
        if ($ren == 'renew') {
            $sql = "UPDATE table_payment_renewal SET trans_status = '1', expiry_date = '$expDate' WHERE portal_id = '$pid' AND trans_status = '0'";
            ...
            $sql = "INSERT INTO transaction_table (portal_id,tin, trans_type, transaction_desc, transaction_amount, payment_mode, posted_ip, created, posted_user, payment_gateway, is_processed,trans_query_id) values ('$portal_id','$tin','$item','Registration', '$amount', 'CARD', '$ip','$date', '$officer', 'MONIFY', '0','$billing_ref')";
    ...
    private function initMonifyPayment($call_back, $brn)
    {
        ...
        if ($err) {
            ...
            $response = array("status" => "failure", "message" => $errorMsg['msg'], "error" => $errorMsg);
        }
