...
$sql_reg_centers = "SELECT * FROM licence_operators";
$reg_centers = $dbobject->db_query($sql_reg_centers);
...
if($user_role == 001)
{
    $sql_role = "SELECT * FROM role WHERE role_id IN ('004','002') ";
}
// else
// {
       ...
       <div class="row" style="<?php echo ($operation == "edit")?"display:none":""; ?>">
        ...
        <div class="row">
           ...
           <div class="col-sm-6">
               <?php
               $display = "none";
               if($operation == "edit" && $user[0]['role_id'] == "002")
               {
                   $display = "block";
               ...
               ?>
               <div class="form-group" id="reg_center_div" style="display:<?php echo $display; ?>">
                    <label class="form-label">Assign to a Registration Center<span class="asterik">*</span></label>
                    <select class="form-control" name="reg_center" id="reg_center">
                        <?php
                            echo '<option value="">::SELECT REGISTRATION CENTERE::</option>';
                            foreach($reg_centers as $row)
                            {
                                $selected = ($user[0]['reg_center'] == $row['id'])?"selected":"";
